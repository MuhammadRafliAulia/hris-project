...
<body>
 ...
 @endphp
 <img src="{{ $optUrl }}" alt="Opsi A" style="max-width:100px;max-height:60px;border-radius:4px;border:1px solid #e2e8f0;">
 ...
 @endphp
 <img src="{{ $optUrl }}" alt="Opsi B" style="max-width:100px;max-height:60px;border-radius:4px;border:1px solid #e2e8f0;">
 ...
 @endphp
 <img src="{{ $optUrl }}" alt="Opsi C" style="max-width:100px;max-height:60px;border-radius:4px;border:1px solid #e2e8f0;">
 ...
 @endphp
 <img src="{{ $optUrl }}" alt="Opsi D" style="max-width:100px;max-height:60px;border-radius:4px;border:1px solid #e2e8f0;">
 ...
 <input type="text" id="option_e" name="option_e" value="{{ old('option_e', $question->option_e) }}" placeholder="Masukkan opsi E">
 @error('option_e')<div class="error">{{ $message }}</div>@enderror
 <div style="margin-top:6px;">
 <label style="font-size:12px;color:#64748b;">Gambar Opsi E (opsional)</label>
 <input type="file" name="option_e_image" accept="image/*" style="font-size:12px;">
 @if($question->option_e_image)
 <div style="margin-top:6px;display:flex;align-items:center;gap:8px;">
 @php
	 $opt = $question->option_e_image;
	 if (filter_var($opt, FILTER_VALIDATE_URL)) {
		 $optUrl = $opt;
	 } elseif (\Illuminate\Support\Str::startsWith($opt, 'storage/')) {
		 $optUrl = asset($opt);
 ...
 @endphp
 <img src="{{ $optUrl }}" alt="Opsi E" style="max-width:100px;max-height:60px;border-radius:4px;border:1px solid #e2e8f0;">
 <label style="font-size:11px;color:#dc2626;cursor:pointer;"><input type="checkbox" name="remove_option_e_image" value="1"> Hapus gambar</label>
 </div>
 @endif
 </div>
 </div>
...
@endphp
<div style="margin-top:12px;">
<img src="{{ $imgUrl }}" alt="Preview" style="max-width:200px; max-height:200px; border-radius:6px;">
</div>
@endif
 </div>
 ...
 @php
	 $aud = $question->audio;
	 if (filter_var($aud, FILTER_VALIDATE_URL)) {
		 $audUrl = $aud;
	 } elseif (\Illuminate\Support\Str::startsWith($aud, 'storage/')) {
		 $audUrl = asset($aud);
	 } else {
		 $audUrl = asset('storage/' . ltrim($aud, '/'));
	 }
 @endphp
 <div style="margin-top:12px;">
